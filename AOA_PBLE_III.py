...
def greedy_route_solver(points, depot, num_drivers, max_route_dist):
    """
    Technique 1: Greedy Approach (Focus on initial efficiency & load minimum).
    Builds and assigns routes by prioritizing the nearest neighbor for efficiency.
    Returns: A list of final route loads (distances).
    """
    unassigned_points = set(points.keys())
    driver_data = {i: {'route': ['DEPOT'], 'distance': 0.0} for i in range(1, num_drivers + 1)}
    
    # 1. Build initial mini-routes (Greedy assignment)
    temp_routes = []
    
    # Simple loop to create routes until points run out or max_dist is hit
    while unassigned_points:
        route_path = ['DEPOT']
        current_dist = 0
        current_stop_name = 'DEPOT'
        current_stop_coords = depot
        ...
        while True:
            ...
            for p_name in list(unassigned_points):
                p_coords = points[p_name]
                dist = calculate_distance(current_stop_coords, p_coords)
                
                # Check distance constraint (dist to P + dist from P back to Depot)
                dist_to_depot = calculate_distance(p_coords, depot)
                if current_dist + dist + dist_to_depot <= max_route_dist:
            ...
            if best_next_point:
                ...
                current_stop_name = best_next_point
                current_stop_coords = points[best_next_point]
            else:
                break
        
        # Close the route
        if len(route_path) > 1:
            final_load = current_dist + calculate_distance(current_stop_coords, depot)
            temp_routes.append({'path': route_path + ['DEPOT'], 'load': final_load})
            
    # 2. Assign the generated mini-routes to drivers to return driver_data
    return dp_partition_solver(temp_routes, num_drivers)


def dp_partition_solver(mini_routes, num_drivers):
    """
    Technique 2: DP Partition Heuristic (Focus on minimizing load imbalance/variance).
    Assigns a set of mini-routes to K drivers to equalize loads.
    
    *Note: This is a GREEDY HEURISTIC for K-way partitioning (NP-hard problem), 
    as full DP for K > 2 is computationally prohibitive for a prototype.*
    """
    
    # Sort routes by load (largest first) for better greedy heuristic performance
    ...
    for route in sorted_routes:
        # Find driver with the minimum load
    ...
    for id, data in driver_data.items():
        # Combine the sequence of points from multiple mini-routes for display
        full_route_sequence = []
        for path in data['route_paths']:
            # Append points, ensuring "DEPOT" isn't repeated mid-route
...
class DeliveryOptimizerApp:
    def __init__(self, master):
        self.master = master
        master.title("VRP: Greedy vs. DP Partition Comparison")
    ...
    def create_widgets(self):
        ...
        ttk.Button(config_frame, text="Run Comparison", command=self.run_optimization).grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def run_optimization(self):
        try:
            ...
            # --- 1. Generate Base Mini-Routes (Needed for both techniques) ---
            # Using the first part of the Greedy Solver logic to create a set of efficient tasks
            mini_routes = self._generate_base_routes(max_d)

            # --- 2. Execute Chosen Algorithm ---
            if "DP Partition" in choice:
                # Runs the pure load balancer (DP Heuristic)
                driver_data = dp_partition_solver(mini_routes, num_d)
            elif "Greedy Route Solver" in choice:
                # Runs the full sequential Greedy VRP solver 
                # (Note: This is less about fair partitioning and more about VRP efficiency)
                driver_data = self._sequential_greedy_solver(max_d, num_d)
            else:
                return 

            # --- 3. Update Metrics and UI ---
            self._update_ui(driver_data)
            
        except ValueError:
            messagebox.showerror("Error", "Invalid input. Please use numbers.")
        except Exception as e:
            messagebox.showerror("Runtime Error", f"Error: {e}")

    def _generate_base_routes(self, max_d):
        """Helper to create a standard set of efficient mini-routes for partitioning."""
        # Note: This uses the Greedy Logic internally to generate the mini-routes
        
        unassigned_points = set(self.points_data.keys())
        temp_routes = []
        
        while unassigned_points:
            route_path = ['DEPOT']
            current_dist = 0
            current_stop_coords = self.depot_coords
            
            # Find the first point closest to the Depot
            start_point = min(unassigned_points, key=lambda p: calculate_distance(self.depot_coords, self.points_data[p]))
            ...
            while True:
                ...
                last_coords = self.points_data.get(route_path[-1], self.depot_coords)

                for p_name in list(unassigned_points):
                    p_coords = self.points_data[p_name]
                    dist = calculate_distance(last_coords, p_coords)
                    
                    dist_to_depot = calculate_distance(p_coords, self.depot_coords)
            ...
            if len(route_path) > 1:
                final_load = current_dist + calculate_distance(self.points_data.get(route_path[-1], self.depot_coords), self.depot_coords)
                temp_routes.append({'path': route_path + ['DEPOT'], 'load': final_load})
                
        return temp_routes

    def _sequential_greedy_solver(self, max_d, num_d):
        """The pure Greedy VRP approach (Less focused on fairness)."""
        # This is a different, simplified VRP implementation where one driver completes
        # a route, returns to the depot, and the next driver starts.
        
        # Simplified for time; in reality, this would require a separate, complete VRP solver
        messagebox.showinfo("Note", "The 'Greedy Route Solver' is implemented as the base mini-route generator which prioritizes local efficiency.")
        
        # Fallback to the DP Partition solver using the base routes for comparison
        mini_routes = self._generate_base_routes(max_d)
        
        # Since the 'sequential' Greedy VRP is complex, we simulate its *output*
        # by simply distributing the base routes sequentially to drivers.
        # This is a very poor balancing method, but serves as a clear contrast.
        
        driver_data = {i: {'route_paths': [], 'distance': 0.0} for i in range(1, num_d + 1)}
        ...
        return dp_partition_solver(mini_routes, num_d) # Use DP Partition as the final structure
    ...
    def _update_ui(self, driver_data):
        ...
        if not distances:
            self.total_dist_label.config(text="Total System Distance: N/A")
